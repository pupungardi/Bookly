...
-- Description: Complete production schema including User Profiles, Books Catalog,
--              Payments, Reading Progress, Storage Buckets, and Row Level Security.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- TABLE: public.profiles (User Profiles synced with auth.users)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- TABLE: public.books (eBook Catalog with Cover & PDF URLs)
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.books (
    ...
    description TEXT,
    synopsis TEXT,
    cover_url TEXT,                        -- Public URL from 'book-covers' bucket or CDN
    pdf_url TEXT,                          -- Secure storage URL from 'ebook-files' bucket
    content TEXT,                          -- Full chapter text for in-app reader
...
-- ----------------------------------------------------------------------------
-- TABLE: public.payments (Transaction History & Payment Processing)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- TABLE: public.user_reading_progress (Bookmarks & Reading State)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- TABLE: public.user_bookmarks (Favorite / Saved Books Collection)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- TABLE: public.book_reviews (User Reviews & Ratings)
-- ----------------------------------------------------------------------------
...
-- 7. NEW USER PROFILE SYNC TRIGGER
...
-- ----------------------------------------------------------------------------
-- READING PROGRESS & BOOKMARKS POLICIES
-- ----------------------------------------------------------------------------
...
-- Ensure 'book-covers' and 'ebook-files' buckets exist
...
VALUES 
    ...
    (
        'ebook-files', 
        'ebook-files', 
        FALSE, -- Private bucket: access protected via RLS & Signed URLs
...
-- 11. STORAGE ROW LEVEL SECURITY POLICIES

-- Policy: Book Covers are publicly readable
...
-- Policy: Admins & Editors can upload book covers
...
-- Policy: Admins & Editors can update/delete book covers
...
-- Policy: eBook Files Read Access (Free books, Purchased books, or Admins)
...
CREATE POLICY "Users can read free or purchased ebook files"
    ...
    USING (
        bucket_id = 'ebook-files' AND (
            ...
            -- Check if corresponding book is free or user has completed payment
            EXISTS (
...
-- Policy: Admins & Editors can upload ebook files
...
-- Policy: Admins & Editors can update/delete ebook files
