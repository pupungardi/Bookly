...
export default function LoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // For now, we'll just redirect back home
    router.push('/');
  };
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex flex-col">
      <header className="bg-white border-b border-stone-100 px-6 py-4 flex items-center gap-4">
        ...
        <h1 className="text-xl font-bold text-stone-900">Masuk</h1>
      </header>
      ...
      <main className="flex-1 flex items-center justify-center p-6">
        ...
        >
          <div className="text-center mb-8">
            ...
            <p className="text-stone-500 text-sm mt-2">Silakan masuk untuk mengakses koleksi buku Anda.</p>
          </div>
          ...
          <p className="text-center mt-6 text-stone-500 text-sm">
            Belum punya akun? <button onClick={() => router.push('/daftar')} className="text-emerald-600 font-bold hover:underline">Daftar di sini</button>
          </p>
        </motion.div>
      </main>
    </div>
