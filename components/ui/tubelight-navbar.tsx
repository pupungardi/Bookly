...
export function NavBar({ items, activeTab, onTabChange, className }: NavBarProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1 bg-white/80 border border-stone-200 backdrop-blur-lg py-1.5 px-1.5 rounded-full shadow-xl">
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <motion.div
                  layoutId="lamp"
                  className="absolute inset-0 w-full bg-emerald-50 rounded-full -z-10"
                  initial={false}
                  transition={{
                    type: "spring",
                    stiffness: 300,
                ...
                >
                  <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-8 h-1 bg-emerald-500 rounded-t-full">
